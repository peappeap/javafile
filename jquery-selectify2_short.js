/*
 * File:        jquery-selectify2_short.js
 * Version:     0.2
 * Author:      Vincent Keizer (www.vicreative.nl)
 * Info:        www.vicreative.nl/projects/selectify2
 *
 ...
 */
(function ($) {
    // Helper methods for selectify2_short
    var helper = {
        checkText: function (select, text) {
            // Checks if current text is still valid for keyboard input.
            if (select.data("selectify2_short").text == text) {
                select.data("selectify2_short").text = "";
            }
        }
    };

    // Events for selectify2_short
    var events = {
                   ...
                   .keydown(function (e) {
                       switch (e.which) {
                           ...
                           case 9:
                               ...
                               break;
                           default:
                               var data = event.data.select.data("selectify2_short");
        ...
        open: function (event) {
            if (!$(this).hasClass("open"));
            {
                ...
                $(this).find(".options").slideDown("fast", function () {
                    $(this).parents(".selectify2_short").addClass("open");
        ...
        change: function () {
            $(this).data("selectify2_short").selectify2_short.trigger("select", $(this).val());
        }
    };
    ...
    var methods = {
        init: function (args) {
            ...
            return $(this).each(function () {
                var $this = $(this);
                var data = $this.data('selectify2_short');

                // If the plugin hasn't been initialized yet
                if (!data) {
                    var select = $('<div />', {
                        "class": 'selectify2_short',
                        "tabindex": "0"
                    ...
                    var cover_select_tag_ele = $("<div />", {
                        "class": "cover_select_tag_ele",
                        click: function () {
                    ...
                    select.append(cover_select_tag_ele.append(selected)
                                        .append(icon))
                          .append(options);
                    $this.before(select);
                    ...
                    $this.data('selectify2_short', {
                        target: $this,
                        selectify2_short: select,
                        text: ""
        ...
        destroy: function () {
            return this.each(function () {
                var $this = $(this),
                    data = $this.data('selectify2_short');
                if (data) {
                    $(window).unbind('.selectify2_short');
                    data.selectify2_short.remove();
                    $this.removeData('selectify2_short')
        ...
        close: function () {
            return this.each(function () {
                var data = $(this).data('selectify2_short');
                if (data) {
                    data.selectify2_short.trigger("removefocus");
                }
            });
        },
        update: function (content) {
            return this.each(function () {
                var data = $(this).data('selectify2_short');
                if (data) {
                    data.selectify2_short.trigger("select", content);
                }
            });
        }
    };

    $.fn.selectify2_short = function (options) {
        var method = options;
        if (methods[method]) {
            ...
            $.error('Method ' + method + ' does not exist on jQuery.selectify2_short');
        }
    };
})(jQuery);
